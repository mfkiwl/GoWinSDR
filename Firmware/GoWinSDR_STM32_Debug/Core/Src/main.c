...
// 	// ad9361_bist_loopback(ad9361_phy, 0); // 0: 关闭环回, 1: �???启环�???

// 	uint8_t data_format = ad9361_spi_read(ad9361_phy->spi, 0x010);
// 	printf("Port Config (0x010): 0x%02X\n", data_format);

// 	data_format = ad9361_spi_read(ad9361_phy->spi, 0x011);
// 	printf("Port Config (0x011): 0x%02X\n", data_format);

// 	// 读取并行接口配置
// 	uint8_t parallel_cfg = ad9361_spi_read(ad9361_phy->spi, 0x012);
// 	printf("Parallel Port (0x012): 0x%02X\n", parallel_cfg);

// 	// 关键：数据接口调谐状�???
// 	uint8_t lvds_cfg = ad9361_spi_read(ad9361_phy->spi, 0x013);
// 	printf("LVDS/CMOS (0x013): 0x%02X (should be 0x00 for CMOS)\n", lvds_cfg);

// 	// RX数据延迟配置
// 	uint8_t rx_delay = ad9361_spi_read(ad9361_phy->spi, 0x006);
// 	printf("RX Data Delay (0x006): 0x%02X\n", rx_delay);

// 	// 数据时钟延迟
// 	uint8_t clk_delay = ad9361_spi_read(ad9361_phy->spi, 0x007);
// 	printf("RX Clock Delay (0x007): 0x%02X\n", clk_delay);
